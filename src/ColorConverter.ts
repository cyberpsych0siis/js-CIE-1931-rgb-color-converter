declare type XY = number[];

declare interface XYObj {
    x: number,
    y: number
}

declare interface Gamut {
    red: XY,
    green: XY,
    blue: XY,
}

declare interface GamutRanges {
    [k: string]: Gamut
}

export default class ColorConverter {
    static getGamutRanges(): GamutRanges {
        let gamutA = {
            red: [0.704, 0.296],
            green: [0.2151, 0.7106],
            blue: [0.138, 0.08]
    ...
    static getLightColorGamutRange(modelId = null){
		let ranges = this.getGamutRanges();
    ...
    static rgbToXy(red: number, green: number, blue: number, modelId = null) {
		...
		red = parseFloat(String(red / 255));
		green = parseFloat(String(green / 255));
		blue = parseFloat(String(blue / 255));
		...
		if(!this.xyIsInGamutRange(xy, colorGamut)){
			xy = this.getClosestColor(xy,colorGamut);
		}

		return xy;
	}

    static xyIsInGamutRange(xy: XYObj, gamut: Gamut) {
    ...
    static getClosestColor(xy: XYObj, gamut: Gamut) {
		function getLineDistance(pointA: XYObj,pointB: XYObj){
			return Math.hypot(pointB.x - pointA.x, pointB.y - pointA.y);
		}

		function getClosestPoint(xy: any, pointA: any, pointB: any) {
